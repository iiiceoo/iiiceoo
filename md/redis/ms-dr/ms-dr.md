...
>Redis 提供了三种集群模式，主从，sentinel 与 cluster。这里聊聊 Redis 主从模式集群应该怎么做容灾。
...
- **复制功能不会阻塞主服务器**：即使有一个或多个从服务器正在进行初次同步（SYNC），主服务器也可以继续处理命令请求，复制采取的是 fork 子进程的形式。
...
Redis 全量同步一般发生在 slave 初始化阶段，这时 slave 需要将 master 上的所有数据都复制一份。具体步骤如下： 
...
- 全量同步结束后，后续 master 收集到的写命令都会通过之前建立的连接，增量发送给 slave 端，slave 重做这些命令以保证数据的更新，称为**增量同步**。
...
这个特性需要主服务器为被发送的复制流创建一个**内存缓冲区**（in-memory backlog），并且主服务器和所有从服务器之间都记录一个复制偏移量（replication offset）和一个主服务器 ID（master run id），当出现网络连接断开时， 从服务器会重新连接，并且向主服务器请求继续执行原来的复制进程：
...
- Redis 2.8 以上版本的该部分重同步特性则用到一个新增命令 `PSYNC`。
...
搭建主从集群，置 173 为 master 节点（A 机房），174（A 机房）、175（B 机房）为 slave 节点。在主节点查看复制信息。
...
手动关闭 A 机房所有节点（173、174），模拟掉电。

![image-20200815154836470](./img/image-20200815154836470.png)

这里选择依次关闭两个节点，其实不太准确，最贴切的测试方案应该是 A 机房的主从节点均部署在同一服务器上，用直接关机来模拟掉电。不过由于 Redis 主从集群没有自动故障转移的能力，对于测试结果没有影响。而在 Redis cluster 集群模式下，若关闭节点有先后顺序，则会得到截然不同的结果。



查看此时 B 机房从节点（175）的信息，显示 A 机房主节点已下线。

![image-20200815155739642](./img/image-20200815155739642.png)



将 B 机房从节点（175）提升为主节点，模拟灾备触发。
...
>**说明**
>
>1. [Redis Keepalived HA](https://iiiceoo.github.io/#/md/redis/keepalived-ha/keepalived-ha) 方案细节。
>2. Redis 集群整体为一主两从，A 机房分布一主一从，B 机房放置一从。主要目标为，**在 A 机房健康时，极大可能的使用 A 机房的 Redis 实例对外提供服务**；当 A 机房完全宕机时，切换至 B 机房提供服务。
>3. A 机房主从间搭建 Keepalived，对外开放一个统一的 `VIP`，VIP 总指向 A 机房主节点的地址。
>4. B 机房的从节点 `slaveof VIP`，即在 A 机房健康的情况下，**它总对当前的主节点进行备份**。
...
>**说明**
>
>1. B 机房从节点执行 `SLAVEOF NO ONE`，将自身提升为主节点。
>2. DNS 切换至灾备场景，B 机房开始对外提供服务。
...
>
>1. A 机房故障恢复。
>2. 通过由 VIP 获得 A 机房对应主节点连接，执行 `SLAVEOF B` 开始进行数据同步。
...
>**说明**
>
>1. 通过 VIP 获得 A 机房对应从节点的连接，执行 `SLAVEOF NO ONE` 提升其自身为主节点。
>2. B 机房主节点执行 `SLAVEOF VIP`，恢复初态。
>3. DNS 切换为默认场景，A 机房恢复对外提供服务。
